...
namespace turtlelib
{
    ...
    TEST_CASE("Normalizing angles")
    {
        ...
        for (int circle_count = -4; circle_count < 4; ++circle_count)
        {
            ...
            for (double base_angle = -PI + rad_step_size; base_angle < PI; base_angle += rad_step_size)
            {
                REQUIRE_THAT(normalize_angle(base_angle + circle_count * 2 * PI), WithinAbs(base_angle, 1e-6));
            }
        }
    };
    ...
    TEST_CASE("Vector2D tests", "[Vector2D]")
    {
        // Setup needed later
